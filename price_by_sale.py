...
def calculate_price_by_sale(priceList, salesPromotions, totalPrice, scannedSaleProducts, product):
    ...
    if p_name in list(priceList.keys()) and p_name in [p.lower() for p in salesPromotions.keys()]:
        ...
        if product.promotions == "b1g1":
            promoStatus = [p.name for p in scannedSaleProducts].count(product.name) >= 1
        elif product.promotions == "b2g1":
            promoStatus = [p.name for p in scannedSaleProducts].count(product.name) >= 2


        if product.promotions == "b1g1" or product.promotions == "b2g1":
            p_stat = "incomplete"
            # Checks if the promo was satisfied, if so apply the discount
            if promoStatus is True:
                product_indices = [i for i, p in enumerate(scannedSaleProducts) if p.name == product.name][:2]
                ...
                if product.promotions == "b1g1":
                    ...
                    del scannedSaleProducts[product_indices[0]]
                    if len(product_indices) > 1: del scannedSaleProducts[product_indices[1] - 1]
